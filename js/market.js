...
$(document).ready(function () {
    ...
    $("#searchInput").on('input', function () {
        ...
        if (this.value.length > 0) {
            // 将this的值转换为小写，这里this指代输入框的输入数据。
            //toLowerCase()将字符串转化成小写，searchText来保存搜索内容
            ...
            $.getJSON("./data/item.json", function (data) {
                ...
                $.each(searchResults, function (index) {
                    ...
                    list-group-item-light list-group-item-action text-decoration-none" 
                    href='./item.html?id=${this.id}'>${this.name}<a/>`);
                });

                // 绑定搜索结果列表中的每个链接，以便在左键单击时导航到相应的页面
                //mousedow事件在定点设备（如鼠标或触摸板）按钮在元素内按下时，会在被绑定的元素上触发
    ...
    // 当搜索框获得焦点时，显示搜索结果列表
    //同理bootstrap里有一个d-none的一个类，利用removeClass给id为search_results删除d-none从而达到隐藏的效果
                    ...
                    $.getJSON("./data/item.json", function (data) {
                        ...
                        //未搜索成功时候弹出的界面
                            ...
                            $(".product_display_area").append(
                                ...
                                <strong style="font-size: 25pt;"><svg xmlns="http://www.w3.org/2000/svg" 
                                fill="#CC3545" height="50" width="50" viewBox="0 0 512 512">
                                <path d="M256 32c14.2 0 27.3 7.5 34.5 19.8l216 368c7.3 12.4 7.3 27.7 .2 40.1S486.3 480 472 
                                480H40c-14.3 0-27.6-7.7-34.7-20.1s-7-27.8 .2-40.1l216-368C228.7 39.5 241.8 32 256 32zm0 
                                128c-13.3 0-24 10.7-24 24V296c0 13.3 10.7 24 24 24s24-10.7 24-24V184c0-13.3-10.7-24-24-24zm32 
