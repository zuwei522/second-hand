...
function msgBox(msgBoxId, type, isRewrite, msg) {
    ...
    if (type == "none") {
        ...
        msgBox.innerHTML = `${isRewrite ? "" : msgBox.innerHTML
            }<div class="alert alert-${type} alert-dismissible"><button type="button" class="close" data-dismiss="alert">&times;</button>${msg}</div>`;
    }
}
...
usrname.onblur = function () {
    if (checkInput(usrname)) {
        ...
        redText(
            usrname,
            "用户名应为<strong>4-15位非数字开头的字母与数字组合</strong>，可使用下划线"
        );
    }
};
...
passwd.onblur = function () {
    ...
    if (checkInput(passwd)) {
        ...
        redText(
            passwd,
            "密码应为<strong>8-16个字符，至少1个大写字母，1个小写字母和1个数字</strong>"
        );
    }
};
